...
-- Question 5: Total number of official languages spoken in each continent
SELECT
    c.Continent,
...
SELECT Continent, AVG(CityPopulation) AS AverageCityPopulation
FROM (
    SELECT c.Continent, ci.Name AS CityName, ci.Population AS CityPopulation
    FROM country c
    JOIN city ci ON c.Code = ci.CountryCode
) AS CityData
GROUP BY Continent
HAVING AVG(CityPopulation) > 200000
ORDER BY AverageCityPopulation DESC;

-- Question 9: Find the total population and average life expectancy for each continent,
-- ordered by average life expectancy in descending order.
SELECT
    Continent,
    SUM(Population) AS total_population,
